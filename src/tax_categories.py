...
def handle(event):
    ...
    if not category_id or not name or rate is None:
        return {"statusCode": 400, "body": json.dumps({"error": "id, name, and rate are required"})}

    try:
        rate_decimal = Decimal(str(rate))
    except InvalidOperation:
        return {"statusCode": 400, "body": json.dumps({"error": "rate must be a number"})}
    ...
    try:
        ...
        return {"statusCode": 500, "body": json.dumps({"error": "failed to save tax category"})}
